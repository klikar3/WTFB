...
*/        }
    ...
    public function actionCreatefast($mId, $grad = 0)
    {
        $pruefung = Pruefungen::find()->where('erledigt = 0')->one();
//        Yii::warning("-----pruefung: ".Vardumper::dumpAsString($pruefung));
        
        $maxgrad = Mitgliedergrade::find()->joinWith('grad')->where(['=', 'MitgliedId', $mId])
                      ->andWhere(['=', 'grade.DispName', $pruefung->disp->DispName])
                      ->orderBy('gradId desc')->one();
        if (!empty($maxgrad)) {              
            $maxgradid = $maxgrad->GradId;
        } else 
            $maxgradid = 0;
        $grad = Grade::find()->where(['=', 'DispName', $pruefung->disp->DispName])
                          ->andWhere(['>', 'gradId', $maxgradid])
                          ->orderBy('gradId')->one();
...
//				Yii::Error( 'test');
        if ($modelp->load(Yii::$app->request->post())) {
            ...
            if ($modelp->print) $model->printed = 1;
...
//		        			return $this->redirect(['/mitgliedergrade/createfast', 'mId' => $mId, 'grad' => $grad]);
		            ...
		            return $this->render('create', [
		                'model' => $modelp, 'ddataProvider' => $ddataProvider, 'gdataProvider' => $gdataProvider, 'grade' => $mgdataProvider, 
		            ]);
						}
		        ...
		        date_default_timezone_set('Europe/Berlin');
		        $mitglied->LetzteAenderung = date('Y-m-d H:i:s');
            $mitglied->PruefungZum = $modelp->GradId;
				  	if (Yii::$app->user->identity->isAdmin) { $mitglied->LetztAendSifu = $mitglied->LetzteAenderung; }
		        $mitglied->save();
